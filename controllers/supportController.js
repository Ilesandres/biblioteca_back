            ...
            const [result] = await pool.query(
                `SELECT t.*, u.nombre as nombreUsuario,
                    ...
                    (SELECT nombre FROM usuario WHERE id = 
                        (SELECT usuarioId FROM support_agent WHERE id = t.agenteId)) as agenteAsignado
                 FROM support_ticket t 
...
const obtenerMensajesTicket = async (req, res) => {
    try {
        ...
        const [mensajes] = await pool.query(
            `SELECT m.*, u.nombre as nombreEmisor
...
const obtenerAgentes = async (req, res) => {
    try {
        const [agentes] = await pool.query(
            `SELECT u.id, u.nombre, u.email, u.rol, sa.estado, sa.id as agenteId
             FROM usuario u
